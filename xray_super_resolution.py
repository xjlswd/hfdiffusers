...
def main():
    ...
    #     torch.load("results/xray_sr_64_256/unet.pt", map_location=torch.device("cpu"))
    # )
    # unet = UNet2DModel.from_pretrained(f"results/xray_sr_64_256/checkpoint-25000/unet_ema")

    # num of parameters
    if accelerator.is_main_process:
        print(f"Number of parameters in the UNet: {sum(p.numel() for p in unet.parameters())/1000000}M")
    ...
    #     torch.load("results/xray_sr_64_256/unet.pt", map_location=torch.device("cpu"))
    # )
        # ema_unet = UNet2DModel.from_pretrained(f"results/xray_sr_64_256/checkpoint-25000/unet_ema")
        # ema_unet = EMAModel(
        #     ema_unet.parameters(),
        #     model_cls=UNet2DModel,
        #     model_config=ema_unet.config,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if accelerator.is_main_process:
            if epoch % args.validation_epochs == 0:
                ...
                lr_images = glob.glob("data/valid/*.png")
                processd_images = []
                hr_imgs = []
