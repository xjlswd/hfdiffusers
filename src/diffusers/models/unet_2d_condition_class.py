...
class UNet2DClassConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
    ...
    def __init__(
        ...
        if class_condition:
            self.class_embedding = nn.Embedding(
                num_class_embeds + 1, cross_attention_dim
            )  # classifier-free
        # unconditional
        else:
    ...
    def forward(
        ...
        if self.class_embedding is not None:
            ...
            encoder_hidden_states = (
                self.class_embedding(class_labels).unsqueeze(1).to(dtype=self.dtype)
            )
