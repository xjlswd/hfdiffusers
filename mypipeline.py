...
class MyPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        self.labels = {}
        self.num_class = (unet.config.num_class_embeds,)
    ...
    def __call__(
        ...
        Args:
            ...
            output_type (`str`, *optional*, defaults to `"pil"`):
                The output format of the generated image. Choose between `PIL.Image`,`tensor`, or `np.array`.
        ...
        latent_model_input = torch.cat([images] * 2) if guidance_scale > 1 else images

        class_labels = torch.tensor(
            class_labels, device=self._execution_device
        ).reshape(-1)
        class_null = torch.tensor(
            [self.num_class] * batch_size, device=self._execution_device
        ).reshape(-1)
        class_labels_input = (
        ...
        if guidance_scale > 1:
            samples, _ = latent_model_input.chunk(2, dim=0)
        else:
            samples = latent_model_input
        samples = 1 / self.vae.config.scaling_factor * samples
